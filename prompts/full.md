...
|                    | single letter systematic    | single letter variable name with some logic, which is however not common for this case (n for number or i for index is ok) | s for student, u for user                                            |
...
These examples illustrate the expected output and its stringency:

```
def censor(text):
    new = ''
    for inx, i in enumerate(text, 1):
        if inx%2 == 0:
            new += 'X'
        else:
            new += i
...
1: text: good
2: new: good
3: inx: medium: uninformative: unnecessary abbreviation
3: i: bad: confusing: contradicting common use

```
def print_powers(base, n):
    power = 1
...
1: base: good
1: n: good
2: power: bad: contradicting meaning
3: i: good
4: base_: bad: confusing: contradicting meaning
